...
static int ext_power_transient_pm_action(const struct device *dev, enum pm_device_action action) {
    switch (action) {
...
#define EXT_POWER_TRANSIENT_DEVICE(idx)                                                            \
    ...
    static const struct gpio_dt_spec                                                               \
        ext_power_##idx##_control_gpios[DT_INST_PROP_LEN(idx, control_gpios)] = {                  \
            DT_INST_FOREACH_PROP_ELEM_SEP(idx, control_gpios, GPIO_DT_SPEC_GET_BY_IDX, (, ))};     \
                                                                                                   \
    ...
    DEVICE_DT_INST_DEFINE(idx, ext_power_transient_init, PM_DEVICE_DT_INST_GET(idx),               \
